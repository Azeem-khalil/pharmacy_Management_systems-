    ...
    <div id="layoutSidenav">
        ...
        <div id="layoutSidenav_content">
            ...
            function FetchAllUsers() {
                ...
                var i = 0;
                var database = firebase.database().ref().child("Pills/Orders/" + koopId);
                database.once("value", function(snapshot) {
                    if (snapshot.exists()) {
                        var content = "";
                        var orderdetail = "";
                        var Phonenumber = snapshot.val().userContactNo;

                        var discount = snapshot.val().discount
                        var discountpercentage = snapshot.val().discountPercentage;
                        var subtotal = snapshot.val().subTotal;
                        ...
                        if (subtotal != null && total != null) {
                            ...
                            content += "<td>" + discount + "</td>"; //column1
                            content += "<td>" + discountpercentage + "</td>"; //column2
                            content += "<td>" + subtotal + "</td>"; //column2
                            content += "<td>" + total + "</td>"; //column2

                            content += "</tr>";


                            snapshot.forEach(function(cart_data) { //For get Cart detail
                                cart_data.forEach(function(child_cart_data) { //get all item of cart

                                    //console.log(child_cart_data.key+": "+child_cart_data.val().name);
                                    var image_link = child_cart_data.val().image;
                                    var name = child_cart_data.val().name;
                                    var mg = child_cart_data.val().mg;
                                    var price = child_cart_data.val().price;
                                    var quantity = child_cart_data.val().quantity;
                                    ...
                                    orderdetail += "<div><img src='" + image_link +
                                        "'class='img-fluid rounded-3' alt='Shopping item' style='width: 65px;'> </div>";
                                    orderdetail +=
                                        "<div class='ms-3'> <h5 style='color:blue'>" + name +
                                        "</h5><p class='small mb-0'>" + mg +
                                        "</p> </div> </div>";
                                    orderdetail +=
